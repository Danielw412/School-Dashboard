...
export function AgentRunsPage() {
  const { data, error, loading } = usePolling(schoolApi.runs, 3_000);
  const [selectedId, setSelectedId] = useState<string | null>(null);
  const selected = data?.find((run) => run.id === selectedId) ?? null;
  return (
    <div className={`page-grid ${selected ? "has-inspector" : ""}`}>
      <section className="page-content runs-page">
        <div className="page-heading-row"><h1>Agent runs</h1></div>
        {error ? <ErrorNotice error={error} /> : null}
        {loading ? <div className="text-skeleton"><span /><span /><span /></div> : null}
        {!loading && !data?.length ? <EmptyState title="No agent runs yet" detail="Start one from an assignment." /> : null}
      ...
      {selected ? (
        <aside className="assignment-inspector run-inspector">
          <div className="inspector-top"><span className="run-icon"><Cpu size={18} /></span><button className="icon-button" aria-label="Close run details" onClick={() => setSelectedId(null)}><X size={18} /></button></div>
          <p className="inspector-course">{featureName(selected.feature)}</p>
          ...
          <Link className="primary-button" to={`/assignment/${encodeURIComponent(selected.logicalId)}?tab=${tabForFeature(selected.feature)}`}>Open result</Link>
          <details className="raw-output"><summary>Structured output</summary><pre>{selected.rawStructuredOutput ?? "No output yet."}</pre></details>
          <details className="raw-output"><summary>Run events</summary><pre>{JSON.stringify(selected.events, null, 2)}</pre></details>
        </aside>
      ) : null}
    </div>
  );
