...
export const defaultSettings: AppSettings = {
  ...
  prompts: {
    ...
    assignmentNavigation: `Use the preloaded assignment context and preflight as authoritative evidence. If preflight says the Directions evidence is sufficient, answer immediately without Canvas or browser retrieval. For agenda/table tasks, use the recovered surrounding row and cells so exact times, submission instructions, required materials, links, and nearby instructions are not lost. If resolution is incomplete, recover context from direct URLs, task/source titles, source text, anchors, and page metadata before one focused search. Use the authenticated Chrome extension only for an already-known linked resource that Canvas cannot read, and never repeat a failed operation with alternate escaping or near-identical wording. Produce only a short student-facing paraphrase, preserve exact problem/page numbers, and stop when the work, submission method, due information, and explicitly referenced instructions are verified.`,
  },
